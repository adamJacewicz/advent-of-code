...
const input = loadInput("./5/input.txt")
...
function processSeed(seed) {
	...
	const ran = maps[processes[activeProcess]].find(r => inRange(seed, r))
	if (!ran) {
		activeProcess++
		return processSeed(seed)
	}
	const diff = seed - ran.source
	activeProcess++
	return processSeed(ran.destination + diff)
}
...
// 	const ranges = seeds.reduce((res, seed, i) => {
// 		if (i % 2 !== 0) {
// 			const arr = Array.from({length: seed}, (_, j) => seeds[i - 1] + j)
// 			return [...res, ...arr]
// 		}
// 		return res
// 	}, [])
// 	return Math.min(...ranges.map(processSeed))
// }
//
